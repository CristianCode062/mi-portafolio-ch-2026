  ...
  useEffect(() => {
    ...
    function draw(now) {
      ...
      for (let b = 0; b < bands; b++) {
        ...
        const gradient = ctx.createLinearGradient(0, height * (0.2 + progress * 0.6), width, height * (0.4 + progress * 0.6));

        // mix colors across stops with slight time-based offsets
        for (let i = 0; i < colorStops.length; i++) {
          const stop = i / (colorStops.length - 1 || 1);
          // slight animated offset
          const offset = stop * 0.4 * Math.sin(t * (0.6 + i * 0.2) + b);
          let color = colorStops[i];
          // create rgba variant with alpha
          gradient.addColorStop(Math.min(1, Math.max(0, stop + offset)), hexToRgba(color, bandAlpha * blend + 0.02));
        }
        ...
        // draw a wavy shape
        ctx.beginPath();
        const waveHeight = 80 * amplitude * (1 + Math.sin(t * (0.2 + b * 0.1)) * 0.6);
        ...
        for (let s = 1; s <= segments; s++) {
          ...
          const y = yBase + Math.sin(t * (1.0 + s * 0.12) + b + s) * waveHeight * Math.cos((s / segments) * Math.PI);
          ctx.lineTo(x, y);
        }
